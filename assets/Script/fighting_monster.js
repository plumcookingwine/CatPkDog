    ...
    onLoad: function () {
        ...
        this.node.on('touchstart', function ( event ) {
            ...
            if(onces === 0 || (kinds === 0 && onces === -1)){
                ...
                var seq = cc.sequence(cc.moveBy(0.5, 0, 100), cc.flipY(true), cc.rotateBy(0.1, -30), cc.moveBy(0.3, 0, -120), finished);
                nodes.runAction(seq);
            }
        });
...
function playAnims(self){
        ...
        var o = cc.random0To1();
        if (o <= 0.25) {
           state = self.anim.playAdditive('monster_anim_top');
        }else if(o > 0.25 && o <= 0.5){
           state = self.anim.playAdditive('monster_anim');
        }else if(o > 0.5 && o < 0.75){
            state = self.anim.playAdditive('monster_anim_b2t');
        }else{
            state = self.anim.playAdditive('monster_anim_b2b');
        }
        state.speed = speed;
        self.scheduleOnce(function(){
            var i = cc.random0To1();
